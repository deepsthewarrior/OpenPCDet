...
#     serialized_object = torch.load(f, map_location=device)
#     print(serialized_object)
# file_path = '/mnt/data/deka01/debug_OpenPCDet/tools/featbank_sh_iou_0.8.pkl.pkl'
# # open the file for reading in binary mode
# with open(file_path, "rb") as file:
#     # load the contents of the file using pickle
#     data = pickle.load(file)
...
# ped_cls = torch.zeros([data['Ped_cls'][0].shape[0]],device=data['Ped_cls'][0].device)
# ped_sh = torch.zeros([data['Ped_cls'][0].shape[0]],device=data['Ped_sh'][0].device)
# ped_reg = torch.zeros([data['Ped_cls'][0].shape[0]],device=data['Ped_reg'][0].device)
# ped_cls_mean = ped_cls.squeeze(dim=0)
# ped_sh_mean = ped_sh.squeeze(dim=0)
# ped_reg_mean = ped_reg.squeeze(dim=0)

# cyc_cls = torch.zeros([data['Cyc_cls'][0].shape[0]],device=data['Cyc_cls'][0].device)
# cyc_sh = torch.zeros([data['Cyc_cls'][0].shape[0]],device=data['Cyc_sh'][0].device)
# cyc_reg = torch.zeros([data['Cyc_cls'][0].shape[0]],device=data['Cyc_reg'][0].device)
# cyc_cls_mean = cyc_cls.squeeze(dim=0) 
# cyc_sh_mean = cyc_sh.squeeze(dim=0)
