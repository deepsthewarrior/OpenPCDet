...
class PredQualityMetrics(Metric):
    ...
    def __init__(self, **kwargs):
        ...
        self.metrics_name = ["pred_ious", "pred_fgs", "sem_score_fgs", "sem_score_bgs", "score_fgs", "pred_ious_bgs",
                             ...
                             "cos_scores_car_sh_bg","cos_scores_car_sh_uc","cos_scores_car_sh_fg","cos_scores_ped_sh_bg","cos_scores_ped_sh_uc",
                             "cos_scores_ped_sh_fg","cos_scores_cyc_sh_bg","cos_scores_cyc_sh_uc","cos_scores_cyc_sh_fg","cos_scores_car_proj_bg","cos_scores_car_proj_uc","cos_scores_car_proj_fg","cos_scores_ped_proj_bg","cos_scores_ped_proj_uc",
                             "cos_scores_ped_proj_fg","cos_scores_cyc_proj_bg","cos_scores_cyc_proj_uc","cos_scores_cyc_proj_fg"]
    ...
    def update(self, preds: [torch.Tensor], ground_truths: [torch.Tensor], pred_scores: [torch.Tensor],
               ...
               cos_scores_car_sh=None,cos_scores_ped_sh=None,cos_scores_cyc_sh=None,cos_scores_softmax=None,
               cos_scores_car_proj=None,cos_scores_ped_proj=None,cos_scores_cyc_proj=None) -> None:
        ...
        for i in range(len(preds)):
            ...
            valid_cos_car_proj = cos_scores_car_proj[i][valid_preds_mask.nonzero().view(-1)] if cos_scores_car_proj else None
            valid_cos_ped_proj = cos_scores_ped_proj[i][valid_preds_mask.nonzero().view(-1)] if cos_scores_ped_proj else None
            valid_cos_cyc_proj = cos_scores_cyc_proj[i][valid_preds_mask.nonzero().view(-1)] if cos_scores_cyc_proj else None
            valid_cos_proj_softmax = cos_scores_softmax[i][valid_preds_mask.nonzero().view(-1)] if cos_scores_softmax else None
            ...
            for cind in range(num_classes):
                ...
                if num_gts > 0 and num_preds > 0:
                    ...
                    if valid_cos_car_sh is not None:
                        cos_scores_car_sh_bg = (valid_cos_car_sh * cls_bg_mask.float()).sum() / cls_bg_mask.float().sum()
                        classwise_metrics['cos_scores_car_sh_bg'][cind] = cos_scores_car_sh_bg
                        cos_scores_car_sh_uc = (valid_cos_car_sh * cc_uc_mask.float()).sum() / cc_uc_mask.float().sum()
                        classwise_metrics['cos_scores_car_sh_uc'][cind] = cos_scores_car_sh_uc
                        cos_scores_car_sh_fg = (valid_cos_car_sh * cc_fg_mask.float()).sum() / cc_fg_mask.sum()
                        classwise_metrics['cos_scores_car_sh_fg'][cind] = cos_scores_car_sh_fg
                    
                    if valid_cos_ped_sh is not None:
                        cos_ped_pool_bg = (valid_cos_ped_sh * cls_bg_mask.float()).sum() / cls_bg_mask.float().sum()
                        classwise_metrics['cos_scores_ped_sh_bg'][cind] = cos_ped_pool_bg
                        cos_ped_pool_uc = (valid_cos_ped_sh * cc_uc_mask.float()).sum() / cc_uc_mask.float().sum()
                        classwise_metrics['cos_scores_ped_sh_uc'][cind] = cos_ped_pool_uc
                        cos_ped_pool_fg = (valid_cos_ped_sh * cc_fg_mask.float()).sum() / cc_fg_mask.sum()
                        classwise_metrics['cos_scores_ped_sh_fg'][cind] = cos_ped_pool_fg
                    
                    if valid_cos_cyc_sh is not None:
                        cos_cyc_pool_bg = (valid_cos_cyc_sh * cls_bg_mask.float()).sum() / cls_bg_mask.float().sum()
                        classwise_metrics['cos_scores_cyc_sh_bg'][cind] = cos_cyc_pool_bg
                        cos_cyc_pool_uc = (valid_cos_cyc_sh * cc_uc_mask.float()).sum() / cc_uc_mask.float().sum()
                        classwise_metrics['cos_scores_cyc_sh_uc'][cind] = cos_cyc_pool_uc
                        cos_cyc_pool_fg = (valid_cos_cyc_sh * cc_fg_mask.float()).sum() / cc_fg_mask.sum()
                        classwise_metrics['cos_scores_cyc_sh_fg'][cind] = cos_cyc_pool_fg

                    if valid_cos_car_proj is not None:
                        cos_scores_car_proj_bg = (valid_cos_car_proj * cls_bg_mask.float()).sum() / cls_bg_mask.float().sum()
                        classwise_metrics['cos_scores_car_proj_bg'][cind] = cos_scores_car_proj_bg
                        cos_scores_car_proj_uc = (valid_cos_car_proj * cc_uc_mask.float()).sum() / cc_uc_mask.float().sum()
                        classwise_metrics['cos_scores_car_proj_uc'][cind] = cos_scores_car_proj_uc
                        cos_scores_car_proj_fg = (valid_cos_car_proj * cc_fg_mask.float()).sum() / cc_fg_mask.sum()
                        classwise_metrics['cos_scores_car_proj_fg'][cind] = cos_scores_car_proj_fg
                    
                    if valid_cos_ped_proj is not None:
                        cos_ped_proj_bg = (valid_cos_ped_proj * cls_bg_mask.float()).sum() / cls_bg_mask.float().sum()
                        classwise_metrics['cos_scores_ped_proj_bg'][cind] = cos_ped_proj_bg
                        cos_ped_proj_uc = (valid_cos_ped_proj * cc_uc_mask.float()).sum() / cc_uc_mask.float().sum()
                        classwise_metrics['cos_scores_ped_proj_uc'][cind] = cos_ped_proj_uc
                        cos_ped_proj_fg = (valid_cos_ped_proj * cc_fg_mask.float()).sum() / cc_fg_mask.sum()
                        classwise_metrics['cos_scores_ped_proj_fg'][cind] = cos_ped_proj_fg
                    
                    if valid_cos_cyc_proj is not None:
                        cos_cyc_proj_bg = (valid_cos_cyc_proj * cls_bg_mask.float()).sum() / cls_bg_mask.float().sum()
                        classwise_metrics['cos_scores_cyc_proj_bg'][cind] = cos_cyc_proj_bg
                        cos_cyc_proj_uc = (valid_cos_cyc_proj * cc_uc_mask.float()).sum() / cc_uc_mask.float().sum()
                        classwise_metrics['cos_scores_cyc_proj_uc'][cind] = cos_cyc_proj_uc
                        cos_cyc_proj_fg = (valid_cos_cyc_proj * cc_fg_mask.float()).sum() / cc_fg_mask.sum()
                        classwise_metrics['cos_scores_cyc_proj_fg'][cind] = cos_cyc_proj_fg
