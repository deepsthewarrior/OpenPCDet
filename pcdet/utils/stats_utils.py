...
class PredQualityMetrics(Metric):
    ...
    def __init__(self, **kwargs):
        ...
        self.metrics_name = ["pred_ious", "pred_fgs", "sem_score_fgs", "sem_score_bgs", "score_fgs", "score_bgs",
                             ...
                             "pred_weight_fn", "pred_weight_tp", "pred_weight_fp", "rcnn_sh_fg_mean","rcnn_sh_uc_mean","rcnn_sh_bg_mean","rcnn_sh_fg_mean_Car","rcnn_sh_fg_mean_Ped","rcnn_cls_fg_mean","rcnn_cls_uc_mean","rcnn_cls_bg_mean",
                             "rcnn_sh_fg_mean_Cyc","rcnn_sh_uc_mean_Car","rcnn_sh_uc_mean_Ped","rcnn_sh_uc_mean_Cyc","rcnn_sh_bg_mean_Ped","rcnn_sh_bg_mean_Cyc","rcnn_cls_fg_mean_Car","rcnn_cls_fg_mean_Ped",
                             "rcnn_cls_fg_mean_Cyc","rcnn_cls_uc_mean_Car","rcnn_cls_uc_mean_Ped","rcnn_cls_uc_mean_Cyc","rcnn_cls_bg_mean_Ped","rcnn_cls_bg_mean_Cyc","rcnn_reg_fg_mean_Car","rcnn_reg_fg_mean_Ped",
                             "rcnn_reg_fg_mean_Cyc","rcnn_reg_uc_mean_Car","rcnn_reg_uc_mean_Ped","rcnn_reg_uc_mean_Cyc","rcnn_reg_bg_mean_Ped","rcnn_reg_bg_mean_Cyc","rcnn_sh_bg_mean_Car","rcnn_cls_bg_mean_Car","rcnn_reg_bg_mean_Car"]
        ...
        for cls in Cls:
            avg = "mean"
            param = "cls"
            self.rcnn_cls_mean.append(self.rcnn_features[cls][avg][param].unsqueeze(dim=0))

        for cls in Cls:
            avg = "mean"
            param = "reg"
            self.rcnn_reg_mean.append(self.rcnn_features[cls][avg][param].unsqueeze(dim=0)) 

        for cls in Cls:
            avg = "mean"
            param = "sh"
            self.rcnn_sh_mean.append(self.rcnn_features[cls][avg][param].unsqueeze(dim=0)) 
    ...
    def update(self, preds: [torch.Tensor], ground_truths: [torch.Tensor], pred_scores: [torch.Tensor],
               ...
               pseudo_labels=None, pseudo_label_scores=None, pred_iou_wrt_pl=None,shared_features=None,rcnn_cls_interim=None,rcnn_reg_interim=None) -> None:
        ...
        for i in range(len(preds)):
            ...
            valid_cls = rcnn_cls_interim[i][valid_preds_mask.nonzero().view(-1)] if rcnn_cls_interim else None
            valid_reg = rcnn_reg_interim[i][valid_preds_mask.nonzero().view(-1)] if rcnn_reg_interim else None
            ...
            for cind in range(num_classes):
                ...
                if num_gts > 0 and num_preds > 0:
                    ...
                    # classwise_metrics['cos_cls_fgs'][cind] = 
...
>>>>>>> 17ec63899b1506d3a65e9ad6df52a6943b2431dc
                        ...
                        cos_sh_bg = torch.stack([F.cosine_similarity(self.rcnn_sh_mean[cind],sh.unsqueeze(dim=0)) for sh in valid_sh[cls_bg_mask]],dim=1) if valid_sh[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_bg_mean'][cind] = cos_sh_bg.mean()
                        cos_sh_uc =  torch.stack([F.cosine_similarity(self.rcnn_sh_mean[cind],sh.unsqueeze(dim=0)) for sh in valid_sh[cc_uc_mask]],dim=1) if valid_sh[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_uc_mean'][cind] = cos_sh_uc.mean()
                        cos_cls_fg = torch.stack([F.cosine_similarity(self.rcnn_cls_mean[cind],sh.unsqueeze(dim=0)) for sh in valid_cls[cc_fg_mask]],dim=1) if valid_cls[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_cls.device)                                               
                        classwise_metrics['rcnn_cls_fg_mean'][cind] = cos_cls_fg.mean()                        
                        cos_cls_bg = torch.stack([F.cosine_similarity(self.rcnn_cls_mean[cind],sh.unsqueeze(dim=0)) for sh in valid_cls[cls_bg_mask]],dim=1) if valid_cls[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_cls.device)
                        classwise_metrics['rcnn_cls_bg_mean'][cind] = cos_cls_bg.mean()
                        cos_cls_uc =  torch.stack([F.cosine_similarity(self.rcnn_cls_mean[cind],sh.unsqueeze(dim=0)) for sh in valid_cls[cc_uc_mask]],dim=1) if valid_cls[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_cls.device)
                        classwise_metrics['rcnn_cls_uc_mean'][cind] = cos_cls_uc.mean()

                        classwise_metrics['rcnn_sh_fg_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[0],sh.unsqueeze(dim=0)) for sh in valid_sh[cc_fg_mask]],dim=1).mean() if valid_sh[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_fg_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[1],sh.unsqueeze(dim=0)) for sh in valid_sh[cc_fg_mask]],dim=1).mean() if valid_sh[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_fg_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[2],sh.unsqueeze(dim=0)) for sh in valid_sh[cc_fg_mask]],dim=1).mean() if valid_sh[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_uc_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[0],sh.unsqueeze(dim=0)) for sh in valid_sh[cc_uc_mask]],dim=1).mean() if valid_sh[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_uc_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[1],sh.unsqueeze(dim=0)) for sh in valid_sh[cc_uc_mask]],dim=1).mean() if valid_sh[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_uc_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[2],sh.unsqueeze(dim=0)) for sh in valid_sh[cc_uc_mask]],dim=1).mean() if valid_sh[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_bg_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[0],sh.unsqueeze(dim=0)) for sh in valid_sh[cls_bg_mask]],dim=1).mean() if valid_sh[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_bg_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[1],sh.unsqueeze(dim=0)) for sh in valid_sh[cls_bg_mask]],dim=1).mean() if valid_sh[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_sh_bg_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_sh_mean[2],sh.unsqueeze(dim=0)) for sh in valid_sh[cls_bg_mask]],dim=1).mean() if valid_sh[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        
                        classwise_metrics['rcnn_cls_fg_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[0],cls.unsqueeze(dim=0)) for cls in valid_cls[cc_fg_mask]],dim=1).mean() if valid_cls[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_cls_fg_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[1],cls.unsqueeze(dim=0)) for cls in valid_cls[cc_fg_mask]],dim=1).mean() if valid_cls[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_cls_fg_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[2],cls.unsqueeze(dim=0)) for cls in valid_cls[cc_fg_mask]],dim=1).mean() if valid_cls[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_cls_uc_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[0],cls.unsqueeze(dim=0)) for cls in valid_cls[cc_uc_mask]],dim=1).mean() if valid_cls[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_cls_uc_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[1],cls.unsqueeze(dim=0)) for cls in valid_cls[cc_uc_mask]],dim=1).mean() if valid_cls[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_cls_uc_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[2],cls.unsqueeze(dim=0)) for cls in valid_cls[cc_uc_mask]],dim=1).mean() if valid_cls[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_cls_bg_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[0],cls.unsqueeze(dim=0)) for cls in valid_cls[cls_bg_mask]],dim=1).mean() if valid_cls[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_cls_bg_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[1],cls.unsqueeze(dim=0)) for cls in valid_cls[cls_bg_mask]],dim=1).mean() if valid_cls[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_cls_bg_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_cls_mean[2],cls.unsqueeze(dim=0)) for cls in valid_cls[cls_bg_mask]],dim=1).mean() if valid_cls[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        
                        classwise_metrics['rcnn_reg_fg_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[0],reg.unsqueeze(dim=0)) for reg in valid_reg[cc_fg_mask]],dim=1).mean() if valid_reg[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_reg_fg_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[1],reg.unsqueeze(dim=0)) for reg in valid_reg[cc_fg_mask]],dim=1).mean() if valid_reg[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_reg_fg_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[2],reg.unsqueeze(dim=0)) for reg in valid_reg[cc_fg_mask]],dim=1).mean() if valid_reg[cc_fg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_reg_uc_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[0],reg.unsqueeze(dim=0)) for reg in valid_reg[cc_uc_mask]],dim=1).mean() if valid_reg[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_reg_uc_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[1],reg.unsqueeze(dim=0)) for reg in valid_reg[cc_uc_mask]],dim=1).mean() if valid_reg[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_reg_uc_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[2],reg.unsqueeze(dim=0)) for reg in valid_reg[cc_uc_mask]],dim=1).mean() if valid_reg[cc_uc_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_reg_bg_mean_Car'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[0],reg.unsqueeze(dim=0)) for reg in valid_reg[cls_bg_mask]],dim=1).mean() if valid_reg[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_reg_bg_mean_Ped'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[1],reg.unsqueeze(dim=0)) for reg in valid_reg[cls_bg_mask]],dim=1).mean() if valid_reg[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)
                        classwise_metrics['rcnn_reg_bg_mean_Cyc'][cind] =   torch.stack([F.cosine_similarity(self.rcnn_reg_mean[2],reg.unsqueeze(dim=0)) for reg in valid_cls[cls_bg_mask]],dim=1).mean() if valid_reg[cls_bg_mask].shape[0] != 0 else torch.full((1,), float('nan'),device=valid_sh.device)                        
                    ...
                    if valid_pred_weights is not None:
                        ...
                        cls_pred_weight_fg = (valid_pred_weights * cls_bg_mask.float()).sum() / cc_fg_mask.sum()
