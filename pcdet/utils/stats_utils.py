...
class PredQualityMetrics(Metric):
    ...
    def __init__(self, **kwargs):
        ...
        self.metrics_name = ["pred_ious", "pred_fgs", "sem_score_fgs", "sem_score_bgs", "score_fgs", "score_bgs",
                             ...
                             "pred_weight_fn", "pred_weight_tp", "pred_weight_fp","cos_scores_fn","cos_scores_fp","cos_scores_tp","rcnn_weights_cls_fg",
                             "rcnn_weights_cls_uc","rcnn_weights_cls_bg","rcnn_weights_cls_fn","rcnn_weights_cls_fp","rcnn_weights_cls_tp"]
    ...
    def update(self, preds: [torch.Tensor], ground_truths: [torch.Tensor], pred_scores: [torch.Tensor],
               rois=None, roi_scores=None, targets=None, target_scores=None, pred_weights=None,cos_scores=None,rcnn_cls_weights=None,
        ...
        for i in range(len(preds)):
            ...
            valid_rcnn_cls_weights = rcnn_cls_weights [i][valid_preds_mask.nonzero().view(-1)] if rcnn_cls_weights else None
            ...
            for cind in range(num_classes):
                ...
                if num_gts > 0 and num_preds > 0:
                    ...
                    if valid_rcnn_cls_weights is not None:
                        cls_rcnn_cls_bg = (valid_rcnn_cls_weights * cls_bg_mask.float()).sum() / cls_bg_mask.float().sum()
                        classwise_metrics['rcnn_weights_cls_bg'][cind] = cls_rcnn_cls_bg
                        cls_rcnn_cls_uc = (valid_rcnn_cls_weights * cc_uc_mask.float()).sum() / cc_uc_mask.float().sum()
                        classwise_metrics['rcnn_weights_cls_uc'][cind] = cls_rcnn_cls_uc
                        cls_rcnn_cls_fg = (valid_rcnn_cls_weights * cc_fg_mask.float()).sum() / cc_fg_mask.sum()
                        classwise_metrics['rcnn_weights_cls_fg'][cind] = cls_rcnn_cls_fg     
                    ...
                    if valid_pred_iou_wrt_pl is not None:
                        ...
                        if valid_rcnn_cls_weights is not None:
                            cls_rcnn_cls_fg_mc = (valid_rcnn_cls_weights * fn_mask.float()).sum() / fn_mask.sum()
                            classwise_metrics['rcnn_weights_cls_fn'][cind] = cls_rcnn_cls_fg_mc
                            cls_rcnn_cls_cc_tp = (valid_rcnn_cls_weights * tp_mask).sum() / tp_mask.float().sum()
                            classwise_metrics['rcnn_weights_cls_tp'][cind] = cls_rcnn_cls_cc_tp
                            cls_rcnn_cls_cc_fp = (valid_rcnn_cls_weights * fp_mask).sum() / fp_mask.float().sum()
                            classwise_metrics['rcnn_weights_cls_fp'][cind] = cls_rcnn_cls_cc_fp 
