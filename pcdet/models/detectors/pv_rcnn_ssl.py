...
class PVRCNN_SSL(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        for cls in self.classes:
            rcnn_sh_mean.append(self.rcnn_features[cls][avg][param].unsqueeze(dim=0).clone().cpu())
        self.rcnn_sh_mean_= torch.stack(rcnn_sh_mean)
        avg = "mean"
        param = "cls"
        for cls in self.classes:
            rcnn_cls_mean.append(self.rcnn_features[cls][avg][param].unsqueeze(dim=0).clone().cpu())
