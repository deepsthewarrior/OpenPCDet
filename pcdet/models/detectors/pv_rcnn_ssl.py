    ...
    def forward(self, batch_dict):
        if self.training:
            ...
            for inds in unlabeled_inds:
                ...
                # temp['projected_features'] = (batch_dict['projected_features'][inds])[:k + 1].clone().detach().cpu().numpy()
                # temp['shared_features'] = (batch_dict['shared_features'][inds]).clone().detach().cpu().numpy()
                temp['shared_features_gt'] = cur_shared_features_gt[:k + 1].clone().detach().cpu().numpy()
                ...
                temp['projected_features_gt'] = (batch_dict['projected_features_gt'][inds])[:k + 1].clone().detach().cpu().numpy()
                ...
                temp['cur_iteration'] = torch.tensor([batch_dict['cur_iteration']]).repeat(unlabeled_inds.shape[0])
