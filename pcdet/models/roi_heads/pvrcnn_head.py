...
class PVRCNNHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict, disable_gt_roi_when_pseudo_labeling=False):
        ...
        if (self.training or self.print_loss_when_eval) and not disable_gt_roi_when_pseudo_labeling: 
            ...
            cos_scores_aggr = []
            for sh in shared_features: 
                cos_scores_classwise = []
                for i in range(3):
                    cos_scores_classwise.append(F.cosine_similarity(temp[i].transpose(1,0),sh.transpose(1,0)))
                cos_scores_aggr.append(cos_scores_classwise)
            cos_sem_scores = torch.Tensor([cos_scores_aggr]).to(rcnn_cls.device).view(batch_dict['roi_labels'].shape[0],-1,3).squeeze(-1)
            targets_dict['cos_sem_scores'] = cos_sem_scores
            targets_dict['cos_scores_softmax'] = F.softmax(cos_sem_scores,dim=-1)
            targets_dict['cos_scores_classifier'] = torch.argmax(targets_dict['cos_scores_softmax'],dim=-1) +1
